...
func CanInline(fn *ir.Func, profile *pgo.Profile) {
	...
	force_func_inl := os.Getenv("GO_FORCE_INLINE_FUNC")
	// fmt.Printf("*V*Z* pro %v\n", force_func_inl)
	...
	if (force_func_inl == fn.Nname.Linksym().Name) {
		fmt.Printf("*V*Z* force function %v\n", fn.Nname)
		budget = int32(2048)
	}
	...
	canInl := !visitor.tooHairy(fn)
	needRetry := false
	// if visitor.tooHairy(fn) {
	// 	reason = visitor.reason
	// 	if (force_func_inl == fn.Nname.Linksym().Name) {
	// 		fmt.Printf("*V*Z* tooHairy reached %v\n", fn.Nname)
	// 	}
	// 	return
	// }

	if (base.Flag.LowerQ == 1 && !canInl) {
		addr := "/tmp/mysocket.sock"

		// Generate a random request ID
		requestID := fn.Nname.Linksym().Name
		...
		// request := "GENERATE " + requestID + " " + 
		request := fmt.Sprintf("MLWRAP_REQ_INL %v %v %v %v %v %v %v %v", fn.Nname.Linksym().Name, budget-visitor.budget, visitor.blocksCount[0], visitor.blocksCount[1],
			visitor.blocksCount[2], visitor.blocksCount[3], visitor.blocksCount[4], visitor.blocksCount[5])
		...
		for scanner.Scan() {
			...
			if strings.HasPrefix(response, requestID) {
				fmt.Printf("Received response: %s\n", response)
				if (strings.HasSuffix(response, "1")) {
					needRetry = true
				}
				break
			}
	...
	if (force_func_inl == fn.Nname.Linksym().Name) {
		fmt.Printf("*V*Z*_force_function_done %v %v %v %v %v %v %v %v\n", fn.Nname, budget-visitor.budget, visitor.blocksCount[0], visitor.blocksCount[1],
			visitor.blocksCount[2], visitor.blocksCount[3], visitor.blocksCount[4], visitor.blocksCount[5])
	}
...
func (v *hairyVisitor) doNode(n ir.Node) bool {
	...
	case ir.OCLOSURE:
		...
		v.budget -= 15
		v.blocksCount[BlockTypeClosure] += 1
	...
	case ir.OAPPEND:
		...
		v.blocksCount[BlockTypeAppend] += 1
	...
	case ir.OFOR:
		v.blocksCount[BlockTypeFor] += 1
	
	case ir.OIF:
		n := n.(*ir.IfStmt)
		v.blocksCount[BlockTypeIf] += 1
